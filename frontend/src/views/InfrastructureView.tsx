...
export const InfrastructureView: React.FC<Props> = ({events,roadSegments,maintenanceQueue}) => {
 ...
 return <div style={{display:'flex',flexDirection:'column',gap:20}}>
  <div><h2 className="heading-md">Signs & crossings Deficiency Intelligence</h2><p style={{color:'var(--text-secondary)',fontSize:'.875rem'}}>Dedicated SIH26124 view for road assets, missing markings, dividers, signboards and waterlogging.</p></div>
  ...
  <div className="panel" style={{padding:16}}><h3 className="heading-sm">Roads needing attention</h3>{deficient.length===0?<p>No roads rated poor or critical in this view.</p>:deficient.map(r=><div key={r.id} style={{display:'flex',justifyContent:'space-between',padding:'12px 0',borderBottom:'1px solid var(--border-subtle)'}}><span><AlertTriangle size={14} style={{verticalAlign:'middle',marginRight:8}}/>{r.road_name}</span><span className="mono">Health {r.condition_score}/100 · {r.defect_count} defects</span></div>)}</div>
 </div>;
};
